...
void TtComboBoxStyle::drawControl(ControlElement element,
  ...
  case QStyle::CE_ShapedFrame: {
    ...
    if (widget->objectName() == "TtComboBoxContainer") {
      ...
      Ui::tTheme->drawEffectShadow(painter, viewRect, shadow_border_width_, 6);
    ...
    if (const QStyleOptionViewItem *vopt =
      ...
      if (option->state & QStyle::State_Selected) {
        if (option->state & QStyle::State_MouseOver) {
          ...
          // 选中
          painter->setBrush(Ui::TtThemeColor(theme_mode_, BasicSelectedAlpha));
          painter->drawPath(path);
        }
        // 选中Mark
        painter->setPen(Qt::NoPen);
      ...
      painter->drawText(textRect, Qt::AlignVCenter | Qt::AlignLeft, elideText);
      painter->restore();
    }
    return;
  }
...
void TtComboBoxStyle::drawComplexControl(ComplexControl control,
  ...
  case QStyle::CC_ComboBox: {
    ...
    if (const QStyleOptionComboBox *copt =
      ...
      // 高度缩小 10 才正常
      // qDebug() << "1: " << copt->rect;
      // qDebug() << subControlRect(CC_ComboBox, copt, SC_ComboBoxFrame,
      // widget);
      ...
      if (expandIconRect.isValid()) {
        ...
        painter->drawText(expandIconRect, Qt::AlignCenter, QChar(0xe653));
        painter->restore();
      }
    }
    return;
...
QRect TtComboBoxStyle::subControlRect(ComplexControl cc,
  ...
  if (cc == QStyle::CC_ComboBox) {
    ...
    case QStyle::SC_ComboBoxArrow: // 箭头区域
      return QRect(rect.right() - 28, rect.top() + (rect.height() - 16) / 2, 16,
                   16);
...
QSize TtComboBoxStyle::sizeFromContents(ContentsType type,
  ...
  case QStyle::CT_ComboBox: {
    ...
    // comboBoxSize += QSize(2 * shadow_border_width_, 2 *
    // shadow_border_width_); 根据字体大小动态调整宽度
    if (option) {
