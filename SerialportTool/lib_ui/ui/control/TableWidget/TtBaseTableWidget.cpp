...
#define BASETABLE_V_SCROLLBAR_STYLE \
  "QScrollBar:vertical{\
...
TtBaseTableWidget::TtBaseTableWidget(QWidget* parent)
    : QTableWidget(parent), verScrollBarSuspensionFlag(false) {
  initTableProperty();
}

TtBaseTableWidget::TtBaseTableWidget(int row, int column, QWidget* parent)
 ...
 */
void TtBaseTableWidget::setItemText(int row, int column, QString text,
                                    int alignment) {
 ...
 */
QString TtBaseTableWidget::getItemText(int row, int column) {
  QTableWidgetItem* item = this->item(row, column);
  QString itemText;
  if (item) {
    itemText = item->text();
 ...
 *@param:   icon:设置的图标
 */
void TtBaseTableWidget::setItemIcon(int row, int column, QIcon& icon) {
  //移除原有的控件,不然有控件的话无法显示文本,没有控件的话该函数不做什么
  this->removeCellWidget(row, column);
  //先获取指定行列的QTableWidgetItem，没有则新建
  QTableWidgetItem* item = this->item(row, column);
 ...
 */
void TtBaseTableWidget::setItemWidget(int row, int column, QWidget* widget,
 ...
 */
QWidget* TtBaseTableWidget::getItemWidget(int row, int column,
                                          bool isCenteredLayout) {
  if (this->cellWidget(row, column) != NULL) {
    QWidget* itemWidget;
    if (isCenteredLayout) {
      itemWidget =
          this->cellWidget(row, column)->findChild<QWidget*>("cellWidget");
      if (itemWidget == 0) {
        return NULL;
      }
    } else {
    ...
    /*返回的是QWidget类型指针，而具体存放的一般是其子类对象,所以使用时需要用
         ...
         * 例:qobject_cast <BaseToolButton *>(TtBaseTableWidget->getItemWidget(2,0));*/
    return itemWidget;
  } else {
    return NULL;
  }
 ...
 */
void TtBaseTableWidget::setItemData(int row, int column, QVariant variant,
                                    int role) {
  //先获取指定行列的QTableWidgetItem，没有则新建
  QTableWidgetItem* item = this->item(row, column);
  if (item) {
 ...
 */
QVariant TtBaseTableWidget::getItemData(int row, int column, int role) {
  QTableWidgetItem* item = this->item(row, column);
  QVariant variant;
  if (item) {
 ...
 */
void TtBaseTableWidget::setColumnAutoNumber(int column, int firstNumber) {
 ...
 *@param:   rowHeight:行高
 */
void TtBaseTableWidget::setColWidthRowHeight(int columnWidth, int rowHeight) {
 ...
 */
void TtBaseTableWidget::setVerScrollBarSuspension(int verScrollBarActualWidth,
                                                  int verScrollBarVisibleWidth,
                                                  bool verScrollBarAsNeeded) {
  verScrollBarSuspensionFlag = true;  //置悬浮标记为true
  this->setVerticalScrollBarPolicy(
      Qt::ScrollBarAlwaysOff);  //关闭垂直滚动条的布局空间占用
  this->verticalScrollBar()->setFixedWidth(verScrollBarActualWidth);
  this->verticalScrollBar()->setParent(this);
  this->verticalScrollBar()->setStyleSheet(
      QString(BASETABLE_V_SCROLLBAR_STYLE)
          .arg(verScrollBarActualWidth -
               verScrollBarVisibleWidth));  //设置默认样式表
  if (verScrollBarAsNeeded)                 //按需显示
  {
    ...
    connect(this->verticalScrollBar(), SIGNAL(rangeChanged(int, int)), this,
            SLOT(verticalScrollBarRangeChangedSlot(int, int)));
  } else  //总是显示
  {
    this->verticalScrollBar()->setVisible(true);
    disconnect(this->verticalScrollBar(), SIGNAL(rangeChanged(int, int)), this,
               SLOT(verticalScrollBarRangeChangedSlot(int, int)));
  }
}
/*
 ...
 *@param:   event:大小事件
 */
void TtBaseTableWidget::resizeEvent(QResizeEvent* event) {
  ...
  if (verScrollBarSuspensionFlag) {
    ...
    this->verticalScrollBar()->move(
        this->width() - this->verticalScrollBar()->width(), 0);
  }
}
/*
 ...
 *@date:    2019.08.26
 */
void TtBaseTableWidget::initTableProperty() {
  ...
  this->setSelectionMode(
      QAbstractItemView::SingleSelection);  //设置选中的模式 单选
  this->setSelectionBehavior(
      QAbstractItemView::SelectRows);  //设置选中的行为 整行选中
 ...
 *@param:   max:最大值
 */
void TtBaseTableWidget::verticalScrollBarRangeChangedSlot(int min, int max) {
  if (min != max && max > 0) {
    this->verticalScrollBar()->setVisible(true);
  } else {
